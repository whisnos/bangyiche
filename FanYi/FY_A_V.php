...
$Wf=R(2,1);
if ($Wz==0){
 $H->ar("<ul class='lb' dl='24,50,a,80,50' c='center,center,left,left,center,center' u='FY_A_V.php'><li class='dhy'></li>");
 $H->ar("  <li class='dh'><span>选择</span><span>编号</span><span>标题</span><span>时间</span><span>操作</span></li>");
 $DB->FCZ();
 $DB->fb="a_dy";
 $DB->fa=10;
 $DB->fp=$Wf;
 $DB->flike=F_HT."FY_A_V.php?/*.html";
 $fz=$DB->FZ();
 ...
 $H->ar("</ul>");
 $H->aj("FZ('#RT .twz','<span>当前位置：</span><span>内容管理</span><span dd=\"a_dy\">单页管理</span><span>列表</span>');");
 $H->aj("FZ('#web .rt','<div class=\"czan\"><a dd=\"g_qx\">全选</a><a dd=\"g_fx\">反选</a><a dd=\"a_xg\">修改</a><a dd=\"a_sc\">删除</a><a dd=\"a_tj\">添加</a></div>".$DB->FY()."');");
 $H->aj("lbwh();");
 $H->aj("lbhover('#web .czan','#Right','FY_A_V.php');");
 $DB->FCZ();
 ...
 $H->aj("FZ('#RT .twz','<span>当前位置：</span><span>内容管理</span><span dd=\"a_dy\">景点管理</span><span>".IIF($Ws<=0,"添加","修改")."</span>');");
 ...
 $H->aj("   $.post('FY_A_V.php?/2.html',{bm:bm,a:a,b:b,c:c,d:d,id:".$Ws."},function(z){gdor('[ bm ]'+z+'！','ok','FY_A_V.php');});");
 $H->aj("})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_cz\"]').click(function(){FCZ('FY_A_V.php?/1_".$Ws."_-3.html')})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_fh\"]').click(function(){FCZ('FY_A_V.php')})");
}elseif($Wz==2){
  ZY();
  $bm=rp("bm");
  $a=rp("a");
