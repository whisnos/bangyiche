<?php
 ...
 $Wf=R(2,0);
if ($Wz==0){
 $H->ar("<ul class='lb' dl='a,24,50,60,150,50' c='left,center,center,center,center,center' u='FY_A_ADVL.php' d='0'><li class='dhy'></li>");
 $H->ar("  <li class='dh'><span>名称</span><span>级别</span><span></span><span></span><span>操作</span></li>");
 function WXL($id,$j,$d){
   global $DB;
   $fz=$DB->Sel("*","a_advl","lid=$id","id desc","");
   $ar="";
   while($R = mysql_fetch_array($fz))
     {
       //预览|内容|增加子类|更改|移动|删除
       $tf=$DB->Setf("a_advl","lid=$R[0]");
 ...
 $H->ar(WXL(0,1,3));
 $H->ar("</ul>");
 $H->aj("FZ('#RT .twz','<span>当前位置：</span><span>内容管理</span><span dd=\"a_adv\">文档管理</span><span dd=\"a_advl\">文档类型</span>');");
 $H->aj("FZ('#web .rt','<div class=\"czan\"><a dd=\"a_tj\">添加顶级</a></div>');");
 $H->aj("lbwh();");
 $H->aj("lbho('#Right','h');");
 }elseif($Wz==1){
  if($Ws>0){$zz=$DB->Sels("id,lm,bh","a_advl","id=$Ws","","");$z=array("","","","",$zz[2],$zz[0],$zz[1]);}
  elseif($Wf>0){$z=$DB->Sels("id,lm,gjz,ms,bh,lid,lmc","a_advl","id=$Wf","id desc","");}
  else{$z=array("","","","","0","0","");}
  
 $H->aj("FZ('#RT .twz','<span>当前位置：</span><span>内容管理</span><span dd=\"a_adv\">文档管理</span><span dd=\"a_advl\">文档类型</span><span>".IIF($Wf<=0,"添加","修改")."</span>');");
 ...
 $H->ar("</ul>");
 $H->aj("bdwh();");
 $H->aj("\$(\".form a[dd='F_lx']\").click(function(){at('FY_A_ADVL_X.php?z=F_lx&id=$Wf','F_lx','150:',$(this))})");
 ...
 $H->aj("   $.post('FY_A_ADVL.php?/2.html',{lm:lm,gjz:gjz,ms:ms,lxid:lxid,lxmc:lxmc,lxbh:lxbh,id:".$Wf."},function(z){gdor('[ lm ]'+z+'！','ok','FY_A_ADVL.php');});");
 $H->aj("})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_cz\"]').click(function(){FCZ('FY_A_ADVL.php?/1_".$Wf."_-3.html')})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_fh\"]').click(function(){FCZ('FY_A_ADVL.php')})");
}elseif($Wz==2){
  ZY();
  $lm=rp("lm");
  $gjz=rp("gjz");
  ...
  if($id==0){
   ...
   $DB->add('a_advl',array('lm','gjz','ms','lid','lmc'),array($lm,$gjz,$ms,$lxid,$lxmc));
  
  $aa=mysql_insert_id();
    $DB->upd('a_advl',array('bh'),array("$lxbh,$aa"),"id=$aa");
    die("成功添加！");
  }elseif($id>0){
  if($lxid>0){
      $lid=$DB->Sels("bh,id","a_advl","id=$lxid","","");
      $ld=$DB->Sels("bh,id","a_advl","id=$id","","");
    if(BH($lid[0],$ld[0])){$lid=array("0","0");}
  }else{
      $lid=array("0","0");
  }
    $DB->upd('a_advl',array('lm','gjz','ms','lid','lmc','bh'),array($lm,$gjz,$ms,$lid[1],$lxmc,$lid[0].','.$id),"id=$id");
    die("成功修改！");
  }
}elseif($Wz==3){
   $ld=$DB->Sels("bh,id","a_advl","id=$Ws","","");
   $DB->del("a_advl","bh like '".$ld[0]."%'");
   ru("FY_A_ADVL.php");
  }
if($Wf!=-3){$H->aj("jzwc();");}
$H->aj("djrt('a_advl','FY_A_ADVL.php');");//这是当前位置的连接
echo $H->x();
