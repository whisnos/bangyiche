...
if ($Wz==0){
 ...
 $H->aj("lbwh();");
 $H->aj("lbhover();");
 $H->aj("FZ('#RT .twz','<span>当前位置：</span><span>内容管理</span><span dd=\"a_pl\">评论管理</span><span>列表</span>');");
 $H->aj("FZ('#web .rt','<div class=".chr(34)."czan".chr(34)."><a>修改</a><a>删除</a><a>全选</a><a>反选</a><a dd=\"a_plt\">添加</a><select name=><option value=-1>所有列表</option>".ZH("<option value=\'{i}\'{t}>{z}</option>"," selected=\'selected\'",-1,$wdlx)."</select><a dd=\'a_pll\'>类型</a></div>".$DB->FY()."');");
 $H->aj("djrtb('a_plt','FY_A_PL.php?/1.html');");
 $H->aj("djrtb('a_pll','FY_A_PLL.php');");
 ...
 $H->aj("FZ('#RT .twz','<span>当前位置：</span><span>内容管理</span><span dd=\"a_pl\">评论管理</span><span>".IIF($Ws<=0,"添加","修改")."</span>');");
