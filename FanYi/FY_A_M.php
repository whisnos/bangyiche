...
$Wf=R(2,1);
if ($Wz==0){
 $H->ar("<ul class='lb' dl='24,50,a,80,50' c='center,center,left,left,center,center' u='FY_A_M.php'><li class='dhy'></li>");
 $H->ar("  <li class='dh'><span>选择</span><span>编号</span><span>标题</span><span>时间</span><span>操作</span></li>");
 $DB->FCZ();
 $DB->fb="a_dy";
 $DB->fa=10;
 $DB->fp=$Wf;
 $DB->flike=F_HT."FY_A_M.php?/*.html";
 $fz=$DB->FZ();
 ...
 $H->aj("lbwh();");
 $H->aj("lbhover('#web .czan','#Right','FY_A_M.php');");
 $DB->FCZ();
 ...
 $H->aj("   $.post('FY_A_M.php?/2.html',{bm:bm,a:a,b:b,c:c,d:d,id:".$Ws."},function(z){gdor('[ bm ]'+z+'！','ok','FY_A_M.php');});");
 $H->aj("})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_cz\"]').click(function(){FCZ('FY_A_M.php?/1_".$Ws."_-3.html')})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_fh\"]').click(function(){FCZ('FY_A_M.php')})");
}elseif($Wz==2){
  ZY();
  $bm=rp("bm");
  $a=rp("a");
