...
if ($Wz==0){
 ...
 $H->ar("</ul>");
 $H->aj("FZ('#RT .twz','<span>当前位置：</span><span>内容管理</span><span dd=\"a_dy\">线路管理</span><span>列表</span>');");
...
//$H->aj("djrtb('a_wdt','FY_A_RD.php?/1.html');");
//$H->aj("djrtb('a_wdl','FY_A_RDL.php');");
 $H->aj("lbwh();");
 $H->aj("lbhover('#web .czan','#Right','FY_A_R.php');");
 $DB->FCZ();
 ...
 $H->aj("\$('#web .rt .czan a[dd=\"a_cz\"]').click(function(){FCZ('FY_A_R.php?/1_".$Ws."_-3.html')})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_fh\"]').click(function(){FCZ('FY_A_R.php')})");
 
 $H->aj("function uping(a){upimg($(a).prev('input'),$(a),'a',4);}");
 $H->aj("function scing(a){scimg($(a),'a',4)}");
 ...
 $H->aj("   $.post('FY_A_R.php?/2.html',{bm:bm,a:a,b:b,c:c,d:d,id:".$Ws."},function(z){gdor('[ bm ]'+z+'！','ok','FY_A_R.php');});");
 $H->aj("})");
