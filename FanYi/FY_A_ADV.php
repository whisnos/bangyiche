<?php
 ...
 ZYs();
 $H=new FY_html();
 $wdlx=CF($DB->arrayz("{z0}||","lm","a_advl","","","","",""),"||");
 $Wz=R(0,0);
 $Ws=R(1,0);
 $Wf=R(2,0);
...
if ($Wz==0){
 $H->ar("<ul class='lb' dl='24,50,a,a,a,80,50' c='center,center,left,left,center,center' u='FY_A_ADV.php' d='$Ws'><li class='dhy'></li>");
 ...
 $DB->FP($Wf);
 if ($Ws>0){
   $dlid=$DB->Sels("id,bh","a_advl","id=$Ws","id desc","");
   $DB->FZW("lbh like'$dlid[1]%'");
 }
 $DB->ft="id,bt,sxz,sxf,sxt,tjdj,lbh,lmc";
 $DB->flike=F_HT."FY_A_ADV.php?/0_".$Ws."_*.html";
 $fz=$DB->FZ();
 ...
 $H->aj("FZ('#web .rt','<div class=\"czan\"><a dd=\"g_qx\">全选</a><a dd=\"g_fx\">反选</a><a dd=\"a_xg\">修改</a><a dd=\"a_sc\">删除</a><a dd=\"a_tj\">添加</a><select dd=\"a_lb\"><option value=\"FY_A_ADV.php\">所有列表</option>".$DB->arrayz("<option value=\"FY_A_ADV.php?/0_{z0}.html\" {t}>{bq}├{z1}</option>","id,lm,bh","a_advl","","","",'selected="selected"',$Ws)."</select><a dd=\'a_lx\'>类型</a></div>".$DB->FY()."');");
//$H->aj("djrtb('a_advt','FY_A_ADV.php?/1.html');");
 $H->aj("djrtb('a_lx','FY_A_ADVL.php');");
 $H->aj("lbwh();");
 $H->aj("XTZ($(\".rt .czan select[dd='a_lb']\"),'#Right');");
 $H->aj("lbhover('#web .czan','#Right','FY_A_ADV.php');");
 $DB->FCZ();
 ...
 if($Wf>0){$z=$DB->Sels("bt,sxz,sxf,sxt,gjz,ms,ly,zz,yls,img,jy,lid,lbh,lmc,lr","a_adv","id=$Wf","id desc","");}
 else{$z=array("","",1,"","","","","","","","","","","","");}
 ...
 $H->aj("\$('#web .rt .czan a[dd=\"a_cz\"]').click(function(){FCZ('FY_A_ADV.php?/1_-3_".$Wf.".html')})");
 $H->aj("\$('#web .rt .czan a[dd=\"a_fh\"]').click(function(){FCZ('FY_A_ADV.php')})");
 ...
 $H->aj("\$(\".form a[dd='F_ly']\").click(function(){at('FY_A_ADVL_X.php?z=F_ly','F_ly','150:',$(this))})");
 $H->aj("\$(\".form a[dd='F_lx']\").click(function(){at('FY_A_ADVL_X.php?z=F_lx','F_lx','150:',$(this))})");
 $H->aj("\$(\".form a[dd='F_zz']\").click(function(){at('FY_A_ADVL_X.php?z=F_zz','F_zz','150:',$(this))})");
 ...
 $H->aj("$.post('FY_A_ADV.php?/2.html',{wdbt:wdbt,wdgjz:wdgjz,wdms:wdms,wdly:wdly,wdzz:wdzz,wdyls:wdyls,wdjy:wdjy,lxmc:lxmc,lxid:lxid,lxbh:lxbh,wdsx0:wdsx0,wdsx1:wdsx1,wdsx2:wdsx2,wdimg:wdimg,wdlr:wdlr,id:".$Wf."},function(z){gdor('[ '+wdbt+' ]'+z+'！','ok','FY_A_ADV.php');});");
 $H->aj("});");
  ...
  $id=rp("id");
  if($id==0){
    $DB->add('a_adv',array('lid','lbh','lmc','bt','gjz','ms','sxz','sxf','sxt','ly','zz','yls','img','jy','lr','tjdj'),array($lid,$lbh,$lmc,$bt,$gjz,$ms,$sxz,$sxf,$sxt,$ly,$zz,$yls,$img,$jy,$lr,date('Y-m-d H:i:s')));
    die("成功添加！");
  }elseif($id>0){
    $DB->upd('a_adv',array('lid','lbh','lmc','bt','gjz','ms','sxz','sxf','sxt','ly','zz','yls','img','jy','lr'),array($lid,$lbh,$lmc,$bt,$gjz,$ms,$sxz,$sxf,$sxt,$ly,$zz,$yls,$img,$jy,$lr),"id=$id");
    die("成功修改！");
  }
}elseif($Wz==3){
  $id=CF(rp("id"),",");
  for($i=0;$i<CFZ($id)-1;$i++){
    $DB->del('a_adv',"id=".$id[$i]);
    
  }die("成功删除！");
}
 
 if($Ws!=-3){$H->aj("jzwc();");}
 $H->aj("djrt('a_adv','FY_A_ADV.php');");
 echo $H->x();
?>